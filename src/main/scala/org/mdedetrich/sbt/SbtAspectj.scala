...
object SbtAspectj extends AutoPlugin {
  ...
  object Ajc {
    ...
    def ajcTask = Def.task {
      ...
      val expanded = (inputs ++ binaries) flatMap { i =>
        if (i.isDirectory) (i ** "*.class").get else Seq(i)
      }
    ...
    def runAjc(inputs: Seq[File],
               sources: Seq[File],
               binaries: Seq[File],
               output: File,
               options: AspectjOptions,
               classpath: Seq[File],
               cacheDir: File,
               log: Logger): Unit = {
    ...
    def ajcOptions(inputs: Seq[File],
                   sources: Seq[File],
                   binaries: Seq[File],
                   output: File,
                   options: AspectjOptions,
                   classpath: Seq[File]): Seq[String] =
    ...
    def logCounts(inputs: Seq[File],
      ...
      if (counts.nonEmpty) {
        log.info("%s %s to %s..." format (running, counts, output))
      }
    }
  ...
  private def compiledClassesTask =
    Def
      .task {
